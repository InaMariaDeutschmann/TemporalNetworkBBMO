...
for(e in env$ID)
{
  dt_temp <- data.frame(Source=as.character(ASV_network$ID), Target=e)
...
```{r}
write.table(NW_filtered[,c("Source","Target")], paste(PATH,"/2_Networks/NW.txt",sep=""),
              col.names = TRUE, row.names = FALSE, sep ="\t", quote = FALSE)
```
...
```{r}
paste(length(which(NW$COMBI_allENV==10)), " (", length(which(NW$COMBI_allENV==10))/dim(NW)[1], "%) edges were kept in network")
paste(length(which(NW$COMBI_allENV<10)), " (", length(which(NW$COMBI_allENV<10))/dim(NW)[1], "%) edges were removed from the network")
```
[1] "26505  ( 0.888832997987928 %) edges were kept in network"
[1] "3315  ( 0.111167002012072 %) edges were removed from the network"
...
length(which(NW$LS>0))/dim(NW)*100
length(which(NW$LS<0))
length(which(NW$LS<0))/dim(NW)[1]*100
dim(NW_filtered)[1]
length(which(NW_filtered$LS>0))
length(which(NW_filtered$LS>0))/dim(NW_filtered)*100
length(which(NW_filtered$LS<0))
...
paste("Euk rads: ", sum(ASV_network[which(substring(ASV_network$ID,1,2)=="ep" | substring(ASV_network$ID,1,2)=="en"),-1])/sum(ASV_sizefilter[which(substring(ASV_sizefilter$ID,1,2)=="ep" | substring(ASV_sizefilter$ID,1,2)=="en"),-1])*100, sep="")
